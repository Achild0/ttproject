<?php
    ...
    */
    ...
    function getRandProduits(){
        ...
        $stmt->execute();
        $res = $stmt->fetchAll();
        $stmt = $_SERVER["pdo"]->prepare("SELECT * FROM photos WHERE produit = :prod");
        $maxres = [];
        foreach($res as $prod){
            $stmt->execute(["prod" => $prod["id"]]);
            $ph = $stmt->fetchAll();    
            foreach($ph as $phot){
            $prod["photos"][] = [$phot["name"],$phot["id"]];
            }
            $maxres[] = $prod;
        }
        return $maxres;
    ...
    function getProduitsByCateg($idcateg){
        ...
        foreach($res as $product){
            ...
            $ph = $stmt->fetchAll();    
            foreach($ph as $phot){
            $product["photos"][] = [$phot["name"],$phot["id"]];
            }
            $catmaxres[] = $product;
        }
        return $catmaxres;
